...
- radar_triple_<res1>_<res2>_<res3>.png/pdf
...
def save_triple_radar(run_root: Path, resolutions: list[str], out_dir: Path, buildings_order: list[str]):
    """Save a 1x3 radar figure for three resolutions (default: 1_hour,30_minutes,5_minutes)."""
    if len(resolutions) != 3:
        raise ValueError(f"triple radar expects exactly 3 resolutions, got: {resolutions}")

    series_list = []
    metrics_list = []
    mp_list = []
    for r in resolutions:
        s, ms, mp = build_series_for_resolution(run_root, r, buildings_order)
        series_list.append(s)
        metrics_list.append(ms)
        mp_list.append(mp)

    # Make the three radars directly comparable by using a shared metric set.
    # Prefer the intersection of PREFERRED_METRICS present in ALL three.
    shared = [m for m in PREFERRED_METRICS if all(m in ms for ms in metrics_list)]
    if not shared:
        # fallback to intersection of whatever is available
        avail_sets = [set(ms) for ms in metrics_list]
        inter = set.intersection(*avail_sets) if avail_sets else set()
        shared = [m for m in PREFERRED_METRICS if m in inter]
    if not shared:
        # ultimate fallback: union (warn by title)
        shared = sorted(list(set().union(*[set(ms) for ms in metrics_list])))

    norms = [normalize_series(s, shared) for s in series_list]

    fig = plt.figure(figsize=(19, 6))
    axes = [fig.add_subplot(1, 3, i + 1, polar=True) for i in range(3)]
    for ax, r, norm in zip(axes, resolutions, norms):
        radar_plot(ax, shared, norm, f"{r} Performance Comparison (Test)")

    # Shared legend
    handles, labels = axes[-1].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper left", bbox_to_anchor=(0.02, 0.98), frameon=False, fontsize=9)
    fig.tight_layout(rect=[0.06, 0.02, 1.0, 0.98])

    out_dir.mkdir(parents=True, exist_ok=True)
    tag = "_".join(resolutions)
    out_png = out_dir / f"radar_triple_{tag}.png"
    out_pdf = out_dir / f"radar_triple_{tag}.pdf"
    fig.savefig(out_png, dpi=300)
    fig.savefig(out_pdf)
    plt.close(fig)

    raw_dump = out_dir / f"radar_triple_{tag}_raw.json"
    raw_dump.write_text(json.dumps({
        "resolutions": resolutions,
        "metrics_json": [str(p) for p in mp_list],
        "raw": series_list,
        "metrics_used": shared,
        "normalization": "min-max per metric across compared series; error-metrics inverted so larger-is-better"
    }, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main():
    ...
    ap.add_argument("--triple", action="store_true",
                    help="If set, also output a 1x3 radar for three resolutions (default: 1_hour,30_minutes,5_minutes).")
    ap.add_argument("--triple_res", type=str, default=",".join(DEFAULT_RESOLUTIONS),
                    help="Comma-separated 3 resolutions for the triple radar.")
    args = ap.parse_args()
    ...
    if args.triple:
        try:
            triple_res = [x.strip() for x in args.triple_res.split(",") if x.strip()]
            png, pdf, raw = save_triple_radar(run_root, triple_res, out_dir, buildings_order)
            print(f"[OK] triple radar: {png}")
            print(f"[OK] triple radar pdf: {pdf}")
            print(f"[OK] raw dump: {raw}")
        except Exception as e:
            print(f"[FAIL] triple radar: {e}")

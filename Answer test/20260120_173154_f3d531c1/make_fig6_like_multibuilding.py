...
def collect_res_to_test_mape(run_root: Path, buildings_order: list[str], resolutions: list[str] | None = None) -> dict:
    """Collect test-set MAPE (0..1) from metrics.json for multiple resolutions."""
    ...
    for r in resolutions:
        ...
        res_to_test_mape[r] = {b: float((tm.get(b, {}) or {}).get("MAPE", np.nan)) for b in buildings_order}
    return res_to_test_mape
...
def make_fig6_like_for_resolution(run_root: Path, resolution: str, out_dir: Path, buildings_order: list[str],
                                 res_to_test_mape: dict | None = None):
    ...
    if res_to_test_mape is None:
        res_to_test_mape = collect_res_to_test_mape(run_root, buildings_order)
    panel_c_resolution_bar(axC, res_to_test_mape, buildings, "(C) Performance Across Resolutions (Test MAPE)")
...
def main():
    ...
    # Pre-collect cross-resolution metrics once (so each 4-panel figure can place the bar chart in quadrant (C))
    res_to_test_mape = collect_res_to_test_mape(run_root, DEFAULT_BUILDINGS, resolutions)

    results = []
    for r in resolutions:
        try:
            info = make_fig6_like_for_resolution(run_root, r, out_dir, DEFAULT_BUILDINGS, res_to_test_mape=res_to_test_mape)
            results.append(info)

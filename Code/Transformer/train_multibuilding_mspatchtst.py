# -*- coding: utf-8 -*-
"""
Multi-building Multi-scale PatchTST with GraphMix + Seasonal-Naive Residual Learning
...
class MultiBuildingMSPatchTST(nn.Module):
    ...
    Output yhat: [B, pred_len, N, Q] if quantiles enabled else [B, pred_len, N, 1]
    """
    def __init__(
        self,
        input_len: int,
        ...
        self.blocks = nn.ModuleList([TransformerBlock(int(d_model), int(n_heads), int(d_ff), float(dropout))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.head(dec)            # [B,pred,N*Q]
        out = out.view(B, self.pred_len, self.N, self.Q)

        # baseline + residual (in scaled space)
        ...
        out = out + base           # residual learning
        return out
...
def try_micro_batch(ds: Dataset, model: nn.Module, device: torch.device, micro_bs: int, use_amp: bool,
                    quantiles: Optional[List[float]]) -> bool:
    ...
    try:
        with autocast_ctx(device, use_amp):
            out = model(xb)  # [B,pred,N,Q]
            if quantiles:
                loss = pinball_loss(yb, out, quantiles)
            else:
                pred = out[:, :, :, 0]
...
def find_micro_batch(ds: Dataset, model: nn.Module, device: torch.device,
                     ...
                     quantiles: Optional[List[float]]) -> int:
    ...
    bs = min_bs
    while bs <= max_bs:
        ok = try_micro_batch(ds, model, device, bs, use_amp, quantiles)
        if ok:
            best = bs
...
def model_cfg_for_resolution(cfg: TrainConfig, resolution: str) -> Dict[str, int]:
    # mild heuristic; adjust if needed
...
def train_one_resolution(
    ...
    if auto_batch and device.type == "cuda" and start_epoch <= 1:
        ...
        cuda_cleanup()
        micro_bs = find_micro_batch(ds_train, model, device, min_batch, max_batch, use_amp, quantiles)
    ...
    print(f"[CFG] {resolution}/{building_tag} | in={wc.input_len} out={wc.pred_len} | "
          f"patch_scales={patch_scales} total_tokens={model.patch.total_tokens} | "
          f"micro_bs={micro_bs} effective={effective_bs} accum={accum_steps} eval_bs={eval_bs} | "
          f"baseline={baseline_mode} adj_init={adj_init} quantiles={(quantiles or [])}")
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        for step, (xb, yb) in enumerate(pbar, start=1):
            ...
            with autocast_ctx(device, use_amp):
                out = model(xb)  # [B,pred,N,Q]
                if quantiles:
                    loss = pinball_loss(yb, out, quantiles) / accum_steps
                else:
                    pred = out[:, :, :, 0]
        ...
        with torch.no_grad():
            for xb, yb in dl_val:
                ...
                with autocast_ctx(device, use_amp):
                    out = model(xb)
                    if quantiles:
                        vloss = pinball_loss(yb, out, quantiles)
                    else:
                        pred = out[:, :, :, 0]

...
def make_grad_scaler(device: torch.device, use_amp: bool):
    ...
    Torch 2.0+ recommends torch.amp.GradScaler('cuda') over torch.cuda.amp.GradScaler.
    We only enable a real scaler on CUDA; on CPU it is a no-op.
    """
...
# -----------------------------
# Calendar (CN holidays) + rolling + mask features
# -----------------------------
@dataclass
class HolidayCalendar:
    """Normalized-day calendar."""
    holidays: set  # set[pd.Timestamp] normalized to 00:00
    workdays: set  # set[pd.Timestamp] normalized to 00:00 (weekend makeup working days)
    source: str
    years: List[int]


def _norm_date_ts(x) -> pd.Timestamp:
    return pd.Timestamp(x).normalize()


def _add_day_range(dst: set, start: str, end: str):
    # inclusive range of days
    for d in pd.date_range(start=start, end=end, freq="D"):
        dst.add(pd.Timestamp(d).normalize())


def build_china_official_calendar_2016_2018() -> HolidayCalendar:
    """Official mainland China holiday arrangements for 2016-2018, including makeup working days.

    Source texts are the State Council General Office holiday arrangement notices.
    This is intentionally limited to 2016-2018 to avoid silently applying an unverified calendar.
    """
    hol: set = set()
    work: set = set()

    # ---- 2016 ----
    hol.add(_norm_date_ts('2016-01-01'))
    _add_day_range(hol, '2016-02-07', '2016-02-13')
    hol.add(_norm_date_ts('2016-04-04'))
    _add_day_range(hol, '2016-05-01', '2016-05-02')
    _add_day_range(hol, '2016-06-09', '2016-06-11')
    _add_day_range(hol, '2016-09-15', '2016-09-17')
    _add_day_range(hol, '2016-10-01', '2016-10-07')

    work.update({_norm_date_ts('2016-02-06'), _norm_date_ts('2016-02-14'),
                 _norm_date_ts('2016-06-12'), _norm_date_ts('2016-09-18'),
                 _norm_date_ts('2016-10-08'), _norm_date_ts('2016-10-09')})

    # ---- 2017 ----
    _add_day_range(hol, '2017-01-01', '2017-01-02')
    _add_day_range(hol, '2017-01-27', '2017-02-02')
    _add_day_range(hol, '2017-04-02', '2017-04-04')
    hol.add(_norm_date_ts('2017-05-01'))
    _add_day_range(hol, '2017-05-28', '2017-05-30')
    _add_day_range(hol, '2017-10-01', '2017-10-08')

    work.update({_norm_date_ts('2017-01-22'), _norm_date_ts('2017-02-04'),
                 _norm_date_ts('2017-04-01'), _norm_date_ts('2017-05-27'),
                 _norm_date_ts('2017-09-30')})

    # ---- 2018 ----
    hol.add(_norm_date_ts('2018-01-01'))
    _add_day_range(hol, '2018-02-15', '2018-02-21')
    _add_day_range(hol, '2018-04-05', '2018-04-07')
    _add_day_range(hol, '2018-04-29', '2018-05-01')
    hol.add(_norm_date_ts('2018-06-18'))
    hol.add(_norm_date_ts('2018-09-24'))
    _add_day_range(hol, '2018-10-01', '2018-10-07')

    work.update({_norm_date_ts('2018-02-11'), _norm_date_ts('2018-02-24'),
                 _norm_date_ts('2018-04-08'), _norm_date_ts('2018-04-28'),
                 _norm_date_ts('2018-09-29'), _norm_date_ts('2018-09-30')})

    return HolidayCalendar(holidays=hol, workdays=work, source='china_official_2016_2018', years=[2016, 2017, 2018])


def load_custom_calendar(path: Path) -> HolidayCalendar:
    """Load custom calendar from JSON.

    Format:
      {"holidays": ["YYYY-MM-DD", ...], "workdays": ["YYYY-MM-DD", ...], "source": "..."}
    """
    obj = safe_json_load(path, default={})
    hol = set(_norm_date_ts(x) for x in obj.get('holidays', []) if str(x).strip())
    work = set(_norm_date_ts(x) for x in obj.get('workdays', []) if str(x).strip())
    src = str(obj.get('source', 'custom'))
    years = sorted({d.year for d in hol.union(work)})
    return HolidayCalendar(holidays=hol, workdays=work, source=src, years=years)


def make_holiday_features(dt_index: pd.DatetimeIndex, cal: HolidayCalendar) -> np.ndarray:
    """Returns [T,3]: is_holiday_off, is_makeup_workday, is_adjacent_to_holiday."""
    if not isinstance(dt_index, pd.DatetimeIndex):
        dt_index = pd.to_datetime(dt_index)
    day = dt_index.normalize()
    is_h = day.isin(list(cal.holidays)).astype(np.float32)
    is_w = day.isin(list(cal.workdays)).astype(np.float32)

    # Adjacent day to any holiday (captures transition effects)
    day_minus = (day - pd.Timedelta(days=1)).isin(list(cal.holidays))
    day_plus = (day + pd.Timedelta(days=1)).isin(list(cal.holidays))
    is_adj = (day_minus | day_plus).astype(np.float32)

    feats = np.stack([is_h, is_w, is_adj], axis=1).astype(np.float32)
    return feats


def parse_rolling_windows(spec: str, steps_per_day: int) -> List[int]:
    """Parse window spec like '3h,1d,7d' into step counts."""
    ...
    for tok in spec.split(','):
        ...
        if not tok:
            continue
        m = re.fullmatch(r"(\d+)([hd])", tok)
        if not m:
            raise ValueError(f"Invalid rolling window token: {tok}. Use like 3h,1d,7d")
        num = int(m.group(1))
        unit = m.group(2)
        if unit == 'h':
            w = int(round(num * steps_per_hour))
        else:
            w = int(round(num * steps_per_day))
        w = max(1, w)
        out.append(w)
    # unique + sorted
    out = sorted(set(out))
...
def make_rolling_features(y_scaled: np.ndarray, windows: List[int], include_std: bool = True,
                          include_diff: bool = True, include_total: bool = True) -> np.ndarray:
    """Causal rolling features computed on *scaled* values.

    Returns [T, F_roll]. Feature order:
      total_mean (optional)
      for each w: per-building roll_mean, per-building roll_std (optional)
      per-building diff_1 (optional)
      for each w: total roll_mean, total roll_std (optional)
    """
    if y_scaled.ndim != 2:
        raise ValueError('y_scaled must be [T,N]')
    T, N = y_scaled.shape
    if not windows and (not include_diff) and (not include_total):
        return np.zeros((T, 0), dtype=np.float32)

    feats = []
    df = pd.DataFrame(y_scaled.astype(np.float32))

    if include_total:
        total = (y_scaled.sum(axis=1, keepdims=True) / max(1, N)).astype(np.float32)
        feats.append(total)
    ...
    for w in windows:
        rm = df.rolling(window=w, min_periods=1).mean().values.astype(np.float32)
        feats.append(rm)
        if include_std:
            rs = df.rolling(window=w, min_periods=1).std().fillna(0.0).values.astype(np.float32)
            feats.append(rs)

    if include_diff:
        diff = np.zeros_like(y_scaled, dtype=np.float32)
        diff[1:, :] = (y_scaled[1:, :] - y_scaled[:-1, :]).astype(np.float32)
        feats.append(diff)

    if include_total and df_total is not None:
        for w in windows:
            trm = df_total.rolling(window=w, min_periods=1).mean().values.astype(np.float32)
            feats.append(trm)
            if include_std:
                trs = df_total.rolling(window=w, min_periods=1).std().fillna(0.0).values.astype(np.float32)
                feats.append(trs)
...
def build_feature_matrix(
    ...
    use_rolling: bool,
    rolling_windows_spec: str,
    add_mask_channels: bool,
    audit_dir: Optional[Path] = None,
) -> np.ndarray:
    """Build full per-timestep exogenous features matrix.

    Output shape: [T, F_total]. Does NOT include y itself.
    """
    if not isinstance(dt_index, pd.DatetimeIndex):
        dt_index = pd.to_datetime(dt_index)

    feats = [make_time_features(dt_index)]

    cal = None
    cm = (calendar_mode or 'off').lower()
    if cm != 'off':
        if cm == 'china_official':
            cal = build_china_official_calendar_2016_2018()
        elif cm == 'custom':
            if calendar_path is None:
                raise ValueError('--holiday_table_path is required when --calendar_mode=custom')
            cal = load_custom_calendar(calendar_path)
        else:
            raise ValueError(f'Unsupported calendar_mode: {calendar_mode}. Use off/china_official/custom')

        # year coverage audit
        years_in_data = sorted(set(int(y) for y in dt_index.year))
        missing_years = [y for y in years_in_data if (cal is not None and y not in cal.years)]
        if missing_years:
            print(f"[WARN] calendar '{cal.source}' lacks years {missing_years}. Those years will have holiday features=0.")

        hf = make_holiday_features(dt_index, cal) if cal is not None else np.zeros((len(dt_index), 0), np.float32)
        # zero out for missing years
        if missing_years and hf.shape[1] > 0:
            m = np.isin(dt_index.year.values, np.array(missing_years, dtype=int))
            hf[m, :] = 0.0
        feats.append(hf)

        if audit_dir is not None and cal is not None:
            audit_dir.mkdir(parents=True, exist_ok=True)
            day = dt_index.normalize()
            hol_in = sorted({str(d.date()) for d in day[day.isin(list(cal.holidays))]})
            work_in = sorted({str(d.date()) for d in day[day.isin(list(cal.workdays))]})
            safe_json_write(audit_dir / 'calendar_audit.json', {
                'calendar_source': cal.source,
                'calendar_years': cal.years,
                'years_in_data': years_in_data,
                'missing_years': missing_years,
                'holiday_days_in_data': hol_in,
                'workday_days_in_data': work_in,
                'n_holiday_days_in_data': len(hol_in),
                'n_workday_days_in_data': len(work_in),
                'timestamp': now_str(),
            })

    if use_rolling:
        wins = parse_rolling_windows(rolling_windows_spec, steps_per_day)
        rf = make_rolling_features(y_scaled, wins, include_std=True, include_diff=True, include_total=True)
        feats.append(rf)

    if add_mask_channels and (obs_mask is not None):
        feats.append(obs_mask.astype(np.float32))
...
def load_multibuilding_splits(
    ...
    impute_method: str = "time",
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray, np.ndarray]:
    """Load train/val/test for multiple buildings.

    Returns:
      train_df, val_df, test_df: filled (no NaN) DataFrames with columns=buildings
      train_mask, val_mask, test_mask: float32 arrays [T,N], 1=observed, 0=imputed (based on pre-fill NaNs)

    Rationale:
      - Outer join can create missing timestamps across buildings.
      - Instead of dropping rows (which may distort temporal alignment), we keep union timestamps and impute.
      - A mask channel allows the model to learn differently on imputed points.
    """
    def load_split(split: str):
        ...
        obs_mask = (~merged.isna()).astype(np.float32).values  # [T,N]
        ...
        if filled.isna().any().any():
            if impute_method == "time":
                ...
                raise ValueError(f"Unknown impute_method: {impute_method}. Use time/linear/ffill")
            filled = filled.ffill().bfill()

        # Final safety
        if filled.isna().any().any():
            nan_rows = filled.isna().any(axis=1)
            n0 = len(filled)
            filled = filled.loc[~nan_rows].copy()
            obs_mask = obs_mask[~nan_rows.values, :]
            print(f"[WARN] {resolution}/{split} still has NaN after impute; dropped rows: {int(nan_rows.sum())} / {n0}")

        return filled, obs_mask

    tr, mtr = load_split("train")
    va, mva = load_split("val")
    te, mte = load_split("test")
    return tr, va, te, mtr, mva, mte
...
class SlidingWindowMultiBuilding(Dataset):
    """
    x: [input_len, C] where C = N(values for buildings) + F(exogenous features)
    y: [pred_len, N]
    y_mask: [pred_len, N], 1=observed, 0=imputed (based on pre-fill NaNs)

    Note:
      Mask channels can also be added into exogenous features via build_feature_matrix(..., add_mask_channels=True).
      y_mask is primarily used to down-weight imputed targets in the loss.
    """
    def __init__(
        self,
        ...
        input_len: int,
        pred_len: int,
        obs_mask: Optional[np.ndarray] = None,  # [T,N] float32
    ):
        ...
        self.mask = obs_mask.astype(np.float32) if (obs_mask is not None) else np.ones_like(self.y, dtype=np.float32)
        assert self.mask.shape == self.y.shape
    ...
    def __getitem__(self, idx: int):
        ...
        fut_y = self.y[s + self.in_len:e, :]           # [out, N]
        fut_m = self.mask[s + self.in_len:e, :]        # [out, N]

        return torch.from_numpy(x), torch.from_numpy(fut_y), torch.from_numpy(fut_m)
    ...
    def future_times(self, idx: int) -> np.ndarray:
        s = int(idx)
        return self.dt[s + self.in_len: s + self.seq_len]
...
def make_loader(ds: Dataset, batch: int, shuffle: bool, num_workers: int, pin: bool, drop_last: bool) -> DataLoader:
    """Create a DataLoader with stable defaults.

    NOTE: This helper existed in v2 but was accidentally omitted in v3.
    Without it, training fails immediately with NameError.
    """
    kw = dict(batch_size=int(batch), shuffle=bool(shuffle), num_workers=int(num_workers),
              pin_memory=bool(pin), drop_last=bool(drop_last))
...
def make_target_weights(y_mask: Optional[torch.Tensor], imputed_target_weight: float) -> Optional[torch.Tensor]:
    """Convert y_mask (1=observed, 0=imputed) into multiplicative weights."""
    if y_mask is None:
        return None
    w = y_mask.float() + (1.0 - y_mask.float()) * float(imputed_target_weight)
    return w


def weighted_smooth_l1(pred: torch.Tensor, target: torch.Tensor, weight: Optional[torch.Tensor], beta: float) -> torch.Tensor:
...
def weighted_pinball(target: torch.Tensor, pred_q: torch.Tensor, quantiles: List[float], weight: Optional[torch.Tensor]) -> torch.Tensor:
    """Pinball loss with optional elementwise weights. pred_q: [B,pred,N,Q]."""
...
def try_micro_batch(ds: Dataset, model: nn.Module, device: torch.device, micro_bs: int, use_amp: bool,
                    ...
                    mean_t: torch.Tensor, scale_t: torch.Tensor, imputed_target_weight: float) -> bool:
    loader = make_loader(ds, micro_bs, shuffle=True, num_workers=0, pin=(device.type == "cuda"), drop_last=True)
    xb, yb, mb = unpack_batch(next(iter(loader)))
    xb = xb.to(device, non_blocking=True)
    yb = yb.to(device, non_blocking=True)
    ...
    # target weights: 1 for observed, <1 for imputed labels (if provided)
    w_tgt = make_target_weights(mb, float(imputed_target_weight)) if mb is not None else None

    try:
        with autocast_ctx(device, use_amp):
            out = model(xb)  # [B,pred,N,Q]
            if quantiles:
                loss_main = weighted_pinball(yb, out, quantiles, w_tgt)
                ...
                pred_point = out[:, :, :, 0]
                loss_main = weighted_smooth_l1(pred_point, yb, w_tgt, beta=float(huber_beta))
...
def find_micro_batch(ds: Dataset, model: nn.Module, device: torch.device,
                     ...
                     quantiles: Optional[List[float]], huber_beta: float, total_loss_weight: float,
                     mean_t: torch.Tensor, scale_t: torch.Tensor, imputed_target_weight: float) -> int:
    ...
    bs = min_bs
    while bs <= max_bs:
        ok = try_micro_batch(ds, model, device, bs, use_amp, quantiles, huber_beta, total_loss_weight, mean_t, scale_t, imputed_target_weight)
        if ok:
            best = bs
...
def train_one_resolution(
    ...
    export_strategy: str,
    explain: bool,
    clip_to_nonneg: bool,    seed: int = 42,
    calendar_mode: str = "off",
    holiday_table_path: str = "",
    use_rolling: bool = True,
    rolling_windows: str = "1d,7d",
    add_mask_channels: bool = True,
    mask_impute: bool = True,
    impute_method: str = "time",
    imputed_target_weight: float = 0.25,
) -> Dict:
    ...
    # Load split data (keep union of timestamps; create obs_mask before imputation)
    train_df, val_df, test_df, m_train, m_val, m_test = load_multibuilding_splits(
    data_root, resolution, buildings,
    impute_missing=bool(mask_impute),
    impute_method=str(impute_method),
    )
    ...
    # Build exogenous features (time + optional CN holiday + optional rolling + optional mask channels)
    cal_audit_root = run_dir / "calendar"
    cal_audit_root.mkdir(parents=True, exist_ok=True)

    cal_path = Path(holiday_table_path) if str(holiday_table_path).strip() else None
    tf_train = build_feature_matrix(
    train_df.index,
    y_scaled=y_train,
    obs_mask=m_train,
    steps_per_day=int(STEPS_PER_DAY[resolution]),
    calendar_mode=str(calendar_mode),
    calendar_path=cal_path,
    use_rolling=bool(use_rolling),
    rolling_windows_spec=str(rolling_windows),
    add_mask_channels=bool(add_mask_channels),
    audit_dir=cal_audit_root / "train",
    )
    tf_val = build_feature_matrix(
    val_df.index,
    y_scaled=y_val,
    obs_mask=m_val,
    steps_per_day=int(STEPS_PER_DAY[resolution]),
    ...
    rolling_windows_spec=str(rolling_windows),
    add_mask_channels=bool(add_mask_channels),
    audit_dir=cal_audit_root / "val",
    )
    tf_test = build_feature_matrix(
    test_df.index,
    y_scaled=y_test,
    obs_mask=m_test,
    steps_per_day=int(STEPS_PER_DAY[resolution]),
    ...
    rolling_windows_spec=str(rolling_windows),
    add_mask_channels=bool(add_mask_channels),
    audit_dir=cal_audit_root / "test",
    )

    safe_json_write(run_dir / "feature_meta.json", {
    "calendar_mode": str(calendar_mode),
    "holiday_table_path": str(holiday_table_path),
    "use_rolling": bool(use_rolling),
    "rolling_windows": str(rolling_windows),
    "add_mask_channels": bool(add_mask_channels),
    "mask_impute": bool(mask_impute),
    "impute_method": str(impute_method),
    "imputed_target_weight": float(imputed_target_weight),
    "timestamp": now_str(),
    })

    ds_train = SlidingWindowMultiBuilding(y_train, tf_train, dt_train, wc.input_len, wc.pred_len, obs_mask=m_train)
    ds_val = SlidingWindowMultiBuilding(y_val, tf_val, dt_val, wc.input_len, wc.pred_len, obs_mask=m_val)
    ds_test = SlidingWindowMultiBuilding(y_test, tf_test, dt_test, wc.input_len, wc.pred_len, obs_mask=m_test)
    ...
    grad_scaler = make_grad_scaler(device, use_amp)
    ...
    if auto_batch and device.type == "cuda" and start_epoch <= 1:
        ...
        cuda_cleanup()
        micro_bs = find_micro_batch(ds_train, model, device, min_batch, max_batch, use_amp, quantiles, cfg.huber_beta, cfg.total_loss_weight, mean_t, scale_t, float(imputed_target_weight))
        ...
        for step, batch in enumerate(pbar, start=1):
            ...
            with autocast_ctx(device, use_amp):
                ...
                if quantiles:
                    ...
                    pred_point = out[:, :, :, 0]
                    loss_main = weighted_smooth_l1(pred_point, yb, w_tgt, beta=float(cfg.huber_beta))
        ...
        with torch.no_grad():
            for batch in dl_val:
                ...
                with autocast_ctx(device, use_amp):
                    out = model(xb)
                    if quantiles:
                        vloss_main = weighted_pinball(yb, out, quantiles, w_tgt)
                        median_idx = int(np.argmin([abs(q - 0.5) for q in quantiles]))
                        pred_point = out[:, :, :, median_idx]
                    else:
                        pred_point = out[:, :, :, 0]
                        vloss_main = weighted_smooth_l1(pred_point, yb, w_tgt, beta=float(cfg.huber_beta))
...
def main():
    ...
    # Calendar / rolling / mask features (minimal interface; enable only after you verify for your dataset)
    ap.add_argument("--calendar_mode", type=str, default="off", choices=["off", "china_official", "custom"],
                help="off: disable holiday features; china_official: built-in Mainland CN official calendars (2016-2018) with make-up workdays; custom: load from JSON")
    ap.add_argument("--holiday_table_path", type=str, default="", help="Path to custom holiday JSON when --calendar_mode=custom")

    ap.add_argument("--use_rolling", action=argparse.BooleanOptionalAction, default=True,
                help="Enable rolling stats features (causal). Use --no-use_rolling to disable.")
    ap.add_argument("--rolling_windows", type=str, default="1d,7d", help="Comma-separated rolling windows, e.g. 1d,7d,3h")

    ap.add_argument("--add_mask_channels", action=argparse.BooleanOptionalAction, default=True,
                help="Append per-building observed-mask channels to exogenous features.")
    ap.add_argument("--mask_impute", action=argparse.BooleanOptionalAction, default=True,
                help="If true, keep union of timestamps and impute missing values; otherwise drop NaN rows.")
    ap.add_argument("--impute_method", type=str, default="time", choices=["time", "linear", "ffill"],
                help="Imputation method used when --mask_impute is enabled.")
    ap.add_argument("--imputed_target_weight", type=float, default=0.25,
                help="Loss weight for imputed target points (0~1). 1 means treat imputed same as observed.")
    ...
    for res in resolutions:
        ...
        try:
            result = train_one_resolution(
                ...
                holiday_table_path=str(args.holiday_table_path),
                use_rolling=bool(args.use_rolling),
                rolling_windows=str(args.rolling_windows),
                add_mask_channels=bool(args.add_mask_channels),
                mask_impute=bool(args.mask_impute),
                impute_method=str(args.impute_method),
                imputed_target_weight=float(args.imputed_target_weight),
            )
